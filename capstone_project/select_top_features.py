...
def select_top_features(train_filepath, test_filepath, feature_importance_filepath, num_features, model_name):
	...
	train_df.to_csv(train_filepath.replace('.csv', '_{0}_top_{1}.csv'.format(model_name, num_features)), index = False)
	test_df.to_csv(test_filepath.replace('.csv', '_{0}_top_{1}.csv'.format(model_name, num_features)), index = False)
